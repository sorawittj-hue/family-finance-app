...
export const WealthCoach = () => {
  const { transactions, wallets, budgets, goals, recurringTxs, currency, cloud } = useFinance();
    ...
    <div className="space-y-6">
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[color:var(--text-secondary)] mt-2 max-w-2xl">
            A decision layer that turns transactions into action: protect cashflow, grow surplus, and remove spending leaks.
          </p>
        </div>
        <div className={`px-4 py-3 rounded-xl border text-xs font-bold ${cloud.isAuthenticated ? toneClass.success : toneClass.warning}`}>
          {cloud.isAuthenticated ? 'Cloud data synced across devices' : 'Sign in on Settings to activate multi-device sync'}
        </div>
      </header>
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-5">
          ...
          <p className="text-xs text-[color:var(--text-secondary)] mt-2">Financial score from cashflow, budget, runway, savings, and debt pressure.</p>
        </Card>
        <Card className="p-5">
          ...
          <p className="text-xs text-[color:var(--text-secondary)] mt-2">Annualized saving and investable surplus engine.</p>
        </Card>
        <Card className="p-5">
          ...
          <p className="text-xs text-[color:var(--text-secondary)] mt-2">Cash needed to reach six months of core expenses.</p>
        </Card>
        <Card className="p-5">
          ...
          <p className="text-xs text-[color:var(--text-secondary)] mt-2">Monthly fixed spending that must earn its place.</p>
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <Card className="xl:col-span-2 p-6">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h2 className="text-lg font-black text-[color:var(--text-primary)]">This month action plan</h2>
              <p className="text-xs text-[color:var(--text-secondary)]">Do these in order. The app will get sharper as your data grows.</p>
            </div>
          </div>
        ...
        <Card className="p-6">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-[color:var(--text-secondary)]">One decision with highest leverage.</p>
            </div>
          </div>
          <div className="space-y-4">
            {system.currentSavingGap > 0 ? (
              <div className="rounded-xl bg-[color:var(--bg-secondary)] border border-[color:var(--border-color)] p-4">
                <p className="text-xs text-[color:var(--text-secondary)]">Automate this amount into savings first</p>
              ...
              <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-4 text-emerald-300">
                ...
                <p className="text-sm font-bold mt-2">Saving discipline is on track. Push surplus toward investments or debt payoff.</p>
              </div>
            )}
            <Button className="w-full gap-2" onClick={() => window.location.assign('/budgets')}>
              Tune budgets <ArrowRight size={14} />
            </Button>
          </div>
        </Card>
      </section>
          ...
          <div className="space-y-3">
            {system.budgetLeaks.length > 0 ? system.budgetLeaks.map((leak) => (
              ...
              <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-4 text-emerald-300 text-sm font-bold">
                No urgent budget leaks detected this month.
              </div>
            )}
          </div>
        </Card>
